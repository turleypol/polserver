...
void handle_secure_trade_msg( Network::Client* client, PKTBI_6F* msg )
{
  ...
  case PKTBI_6F::ACTION_CANCEL:
    INFO_PRINT_TRACE2( 5 )( "Cancel secure trade" );
  ...
  case PKTBI_6F::ACTION_STATUS:
    ...
    if ( set )
    {
      INFO_PRINT_TRACE2( 5 )( "Set secure trade indicator" );
    }
    else
    {
      INFO_PRINT_TRACE2( 5 )( "Clear secure trade indicator" );
    }
    change_trade_status( client->chr, set );
    break;

  default:
    INFO_PRINT_TRACE2( 5 )( "Unknown secure trade action: {}", (int)msg->action );
    break;
  }
}
