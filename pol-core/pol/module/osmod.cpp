...
BObjectImp* OSExecutorModule::mf_SysLog()
{
  ...
  if ( log_verbose )
  {
    ...
    {
      INFO_PRINT2( "{}syslog [{}]: {}{}", color->value(), exec.scriptname(), strval,
                   Clib::Logging::CONSOLE_RESET_COLOR );
    }
    else
    {
      INFO_PRINT2( "syslog [{}]: {}", exec.scriptname(), strval );
    }
  }
  else
  {
    ...
    {
      POLLOG << strval << "\n";
      INFO_PRINT2( "{}{}{}", color->value(), strval, Clib::Logging::CONSOLE_RESET_COLOR );
    }
    else
    {
      POLLOG_INFO << strval << "\n";
  ...
  else  // not being waited for, so queue for later.
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          INFO_PRINT2( "Event queue for {} is full, discarding event.", exec.scriptname() );
          ExecutorModule* em = exec.findModule( "npc" );
          if ( em )
          {
            NPCExecutorModule* npcemod = static_cast<NPCExecutorModule*>( em );
            INFO_PRINT2( "NPC Serial: {:#x}{}", npcemod->controlled_npc().serial,
                         npcemod->controlled_npc().pos() );
          }

          INFO_PRINT2( "Event: {}", ob->getStringRep() );
        }
